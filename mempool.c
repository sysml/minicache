...
#define DV(x) printf( "@%p: " STRINGIFY(x) " = 0x%llx\n", &x, (uint64_t) x)
...
struct mempool *alloc_mempool(uint32_t nb_objs, size_t obj_size, size_t obj_data_align, size_t obj_headroom, size_t obj_tailroom, void (*obj_init_func)(struct mempool_obj *, void *), void *obj_init_func_argp, size_t obj_private_len)
{
  ...
  ASSERT(obj_data_align <= PAGE_SIZE);
  if (obj_data_align)
  ...
  /* allocate pool */
  p = _xmalloc(h_size + (o_size * nb_objs), PAGE_SIZE);
  if (!p) {
	errno = ENOMEM;
	goto error;
  }
  ...
  printf("memory pool resides at %p with %llu kbytes for %llu objects\n", p, (h_size + (o_size * nb_objs)) / 1024, nb_objs);
